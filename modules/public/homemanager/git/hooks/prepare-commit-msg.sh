...
if ! commit_msg=$(git --no-pager diff -U999999 --cached | llm -m gemini-flash-latest -s "$(cat "${HOME}"/.config/prompt/git-commit-msg.md)" 2>&1); then
...
printf "${BLUE}=== Generated Commit Message ===${NC}\n"
printf "${GREEN}$commit_msg${NC}"
printf "\n${BLUE}=================================${NC}"
echo
